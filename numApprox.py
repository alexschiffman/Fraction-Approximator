...
class numApprox:
	def init(self):
		...
		if self.calc(number):
			print self.calc(number)

		elif self.calc(number/pi):
			print self.calc(number/pi), "*pi"
		elif self.calc(number*pi):
			print self.calc(number*pi), "/pi"

		elif self.calc(number/rt2):
			print self.calc(number/rt2), "*root 2"

		elif self.calc(number/rt3):
			print self.calc(number/rt3), "*root 3"


		else:
			print False

	def calc(self, x, error=0.0000000000001):
		n = int(math.floor(x))
		x -= n
		if x < error:
			return (n)
